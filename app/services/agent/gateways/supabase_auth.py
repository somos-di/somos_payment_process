...
class SupabaseAuthGateway:
    ...
    async def get_fresh_jwt(self, access_token: str | None, refresh_token: str | None) -> str:
        exp = self._jwt_exp(access_token) if access_token else None
        if access_token and exp and (exp - time.time()) > self._LEEWAY_SECONDS:
            return access_token

        if not refresh_token:
            raise PermissionError("Sessão ausente ou expirada. Faça login novamente.")

        anon = self.settings.supabase_anon_key.get_secret_value()
